...
def compare_text(pdf_text, html_text):
    ...
    # Extract main question text (assuming it's the longest sentence)
    pdf_sentences = pdf_normalized.split(".")
    html_sentences = html_normalized.split(".")
    pdf_main = max(pdf_sentences, key=len)
    html_main = max(html_sentences, key=len)

    # Compare main content
    if pdf_main == html_main:
        return "Main content is identical"
    else:
        return f"Main content differs:\nPDF: {pdf_main}\nHTML: {html_main}"
...
for result in results:
    ...
    print("Diff:")
    for line in result["differences"]["text_differences"]:
        if line.startswith("+"):
