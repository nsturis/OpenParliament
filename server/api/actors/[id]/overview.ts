...
export default defineEventHandler(async (event): Promise<OverviewResponse> => {
  ...
  const [speechCount, caseCount, current, recentVotesRes, recentSpeechesRes] = await Promise.all([
    ...
    db.execute<Membership>(sql`
      ...
      -- live batch; DISTINCT ON collapses duplicate rows per group.
      SELECT DISTINCT ON (aa."tilaktørid") aa.id, aa."tilaktørid" AS gruppeid, g.navn AS gruppe,
      ...
      ORDER BY aa."tilaktørid", aa.startdato DESC NULLS LAST`),
